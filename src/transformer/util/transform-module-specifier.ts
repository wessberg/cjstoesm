...
export function transformModuleSpecifier (moduleSpecifier: string, {context, parent, resolvedModuleSpecifier}: TransformModuleSpecifierOptions): string {
	...
	return setExtension(
		ensureHasLeadingDotAndPosix(relative(dirname(parent), resolvedModuleSpecifier)),
		determineNewExtension(extname(resolvedModuleSpecifier))
	);
}
